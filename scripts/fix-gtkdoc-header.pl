#!/usr/bin/perl -w
#
# Edit rdf_log.h so that gtk-doc is happy about it
#
# USAGE:
#   perl fix-gtkdoc-header.pl < rdf_log.h > filtered_rdf_log.h
#
...
while(<>) {
  # Remove calling convention macro
  s/REDLAND_CALLBACK_STDCALL *//;

  # Remove trailing macros
  s{REDLAND_PRINTF_FORMAT\(\d+, \d+\);}{;};
