                               ...
                               ) 
{
        ...
        hash=(librdf_hash_factory*)LIBRDF_CALLOC(librdf_hash_factory,
                                                 sizeof(librdf_hash_factory), 1);
        if(!hash)
...
librdf_get_hash_factory (const char *name) 
{
        ...
        if(!name) {
                ...
                if(!factory) {
                        LIBRDF_DEBUG2(librdf_get_hash_factory,
                                      "No hash with name %s found\n",
              name);
                        return NULL;
                }
        }
...
{
        return librdf_hash_get_seq(hash, key, key_len,
                                   LIBRDF_HASH_SEQUENCE_FIRST);
}
        ...
        while(*p) {
                ...
                case RHS_STATE_VALUE:
                        while(*p) {
                                ...
                                new_value=(char*)LIBRDF_MALLOC(cstring,
                                                               real_value_len);
                                if(!new_value)
                                        return 1;
                                ...
                                librdf_hash_put(hash, key, key_len, new_value,
                                                real_value_len, 0);
                ...
                default:
                        LIBRDF_FATAL2(librdf_hash_from_string,
