  ...
  if(opts.database && !strcmp(opts.database, "memory")) {
    ...
    storage_options =
      (char *) malloc(strlen(opts.host) + strlen(opts.database) +
		      strlen(opts.user) + strlen(opts.password) + 120);
    ...
    sprintf(storage_options,
	    "new='%s',host='%s',database='%s',port='%i',user='%s',password='%s',write='yes'",
	    opts.is_new, opts.host, opts.database, opts.port, opts.user,
	    opts.password);
  }
  else {
    ...
    sprintf(storage_options,
	    "new='%s',hash-type='bdb',dir='%s',contexts='yes',write='yes'",
